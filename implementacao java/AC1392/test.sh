...
cdMunicipio=$(cat cdMunicipio.txt)
ant run -DcdMunicipio=$cdMunicipio > $tempFile 2> /dev/null
exitcode=$?
	...
	output=$outFile
		...
		if [ -f $srcDir/$output ]; then
			...
			if [ $? != 0 ]; then
				...
				echo "[E] Testando $dir: |  teste ${testId}, diferenças (professor <- -> aluno):"
				cat $tempfile
				echo ""
			else
			...
			# Reports this error.
			echo "[E] Testando $dir: |  teste ${testId}, não houve saída!"
		fi
else
